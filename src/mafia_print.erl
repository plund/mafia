...
print_trackerI(PP, G, [#mafia_day{votes = Votes0,
    ...
    FmtVoter = "Voter ~s\n",
    FmtTime = "Time  ~s\n",
    io:format(PP#pp.dev, FmtVoter, [pr_ivs_user(IterVotes, PrAbbr)]),
    io:format(PP#pp.dev,
              FmtTime,
    ...
    lists:foldl(
      fun({User, V = #vote{}}, IVs) ->
              {NewIVs, PrIVs} =
                  if V#vote.valid ->
                          NewVote = PrAbbr(b2l(V#vote.vote)),
                          IVs2 =
